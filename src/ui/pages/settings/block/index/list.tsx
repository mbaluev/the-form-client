...
import { Item } from '@ui/pages/settings/block/index/item';
import { Filter } from '@ui/pages/settings/block/index/filter';
...
export const BlocksList = observer(() => {
  ...
  const handleCreate = async () => {
    ...
    if (moduleId) query.moduleId = moduleId;
    await router.push({
      pathname: ROUTES.SETTINGS_BLOCK_CREATE.path,
      query,
    });
  };
      ...
      <Stack flexGrow={1} overflow="hidden">
        <List
          ...
          rowStyleGetter={(item: IBlockDTO) => {
            if (router.pathname === ROUTES.SETTINGS_BLOCK.path) {
